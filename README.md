...
Desktop app for App Store–style promo art: animated backgrounds, iPhone device frames, screen image or video, draggable text and watermarks, and export to PNG, JPEG, or MP4 at several aspect ratios. The UI can be switched between **English** and **中文** in the left sidebar.
...
- **Custom background via Python** — In the background list, choose **Custom code**. The editor runs a drawing script each frame, same idea as the built-in generators:
...
- **iPhone** — Model and color, position and scale as % of the canvas; optional show/hide; **Center phone** snaps to the middle.
...
**Virtual environment — recommended, not required.** A venv avoids conflicting with system or other projects. TangiPromo does **not** depend on a venv; any Python environment where you install the dependencies is fine.
...
用于制作类似 App Store 宣传物料的桌面工具：动态背景、iPhone 外框、屏幕图片或视频、可拖拽的文字与水印，以及多种比例下导出 PNG、JPEG 或 MP4。界面语言可在左侧 **English** / **中文** 之间切换。
...
- **iPhone** — 机型与颜色、位置与缩放（画布百分比）、显示开关；**一键居中** 将手机置于画布中心。
- **屏幕内容** — 为屏幕区域加载静态图或视频。
...
**虚拟环境：建议使用，不强制。** 使用 venv 可减少与系统或其它项目的依赖冲突；程序本身**不要求**必须用虚拟环境，只要在同一个 Python 环境里装好依赖即可。
